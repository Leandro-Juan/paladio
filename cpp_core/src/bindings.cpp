...
PYBIND11_MODULE(paladio_core, m) {
    ...
    py::enum_<NodeType>(m, "NodeType", "Semantic categories for Points of Interest.")
    ...
    py::class_<TransitInfo>(m, "TransitInfo", "Travel edge connecting two POIs.")
        ...
        .def_readwrite("duration", &TransitInfo::duration, "Travel duration in minutes.")
    ...
    py::class_<OptimizationConfig>(m, "OptimizationConfig", "Global constraints for the routing problem.")
        ...
        .def_readwrite("breakfast_deadline", &OptimizationConfig::breakfast_deadline)
        ...
        .def_readwrite("idle_time_penalty_rate", &OptimizationConfig::idle_time_penalty_rate)
        .def_readwrite("max_active_time_before_fatigue", &OptimizationConfig::max_active_time_before_fatigue)
    ...
    m.def("optimize_itinerary", [](const std::vector<POI>& pois,
        ...
        if (dur_buf.ndim != 1 || cost_buf.ndim != 1 || dur_buf.shape[0] != cost_buf.shape[0]) {
            throw std::runtime_error("Transit matrices must be flattened 1D arrays of the same length.");
        }
        
        size_t expected_size = pois.size() * pois.size();
        if (dur_buf.shape[0] != static_cast<py::ssize_t>(expected_size)) {
            throw std::runtime_error("Transit matrices must have size exactly equal to N*N where N is the number of POIs.");
        }
