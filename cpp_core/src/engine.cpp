...
inline double calculate_optimistic_bound(uint64_t visited_mask,
  ...
  while (unvisited) {
    ...
    int arrival =
        std::max(current_time + min_transit_global, pois[i].earliest_time);
...
void dfs(int u, SearchState &state, const POI *pois,
         const int *transit_durations, const double *transit_costs,
         const OptimizationConfig &config, const int *sorted_pois_by_density,
         const int *density_rank, int min_transit_global, int n,
  ...
  for (auto it = bucket.begin(); it != bucket.end();) {
    ...
    bool old_dominates =
        (state.current_time >= m.current_time &&
  ...
  for (int i = 0; i < n; ++i) {
    if ((state.visited_mask & (1ULL << i)) == 0) {
      ...
      int arrival_time_before_wait =
          state.current_time + transit_durations[u * n + v];
      ...
      if (is_rest_node) {
        ...
        next_continuous_active_time +=
            transit_durations[u * n + v] + pois[v].duration;
      }
      ...
      dfs(v, state, pois, transit_durations, transit_costs, config,
          sorted_pois_by_density, density_rank, min_transit_global, n, memo,
          global_mandatory_mask, best_result, start_time, node_eval_count);
...
OptimizationResult optimize_itinerary(const std::vector<POI> &pois,
  ...
  if (n > 1) {
    ...
    for (int u = 0; u < n; ++u) {
      for (int v = 0; v < n; ++v) {
        if (u != v) {
          min_transit_global =
              std::min(min_transit_global, transit_durations[u * n + v]);
        }
      }
    }
  }
  ...
  for (int start_node = 0; start_node < n; ++start_node) {
    ...
    try {
      dfs(start_node, state, pois_ptr, transit_durations, transit_costs, config,
          sorted_pois_ptr, density_rank_ptr, min_transit_global, n, memo,
          global_mandatory_mask, best_result, start_time, node_eval_count);
