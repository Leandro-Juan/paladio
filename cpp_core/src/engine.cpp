...
struct SearchState {
  ...
  std::array<uint8_t, 8> category_visits = {
      0}; ///< Count of visits per NodeType.
  std::array<int, 64>
      current_path; ///< Stack-allocated sequence of visited node indices.
...
struct MemoKey {
  ...
  bool operator==(const MemoKey &o) const {
    return mask == o.mask && node == o.node;
  }
};
...
struct MemoKeyHash {
  size_t operator()(const MemoKey &k) const {
    size_t seed = 0;
    seed ^=
        std::hash<uint64_t>()(k.mask) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
...
constexpr size_t TT_SIZE =
    1048576; // 1M entries, limits memory to ~180MB to prevent OOM
 ...
 *
 * Uses the Continuous Fractional Knapsack greedy approach as an admissible
 * heuristic. It assumes we can fractionally visit remaining unvisited POIs
 * ordered by their value-density (score/duration), ignoring travel time. If
 * this optimistic score combined with the current score is worse than the best
 * known score, we can prune.
 *
 ...
 * @param sorted_pois_by_density Indices of POIs sorted descending by
 * score/duration density.
 * @return double The maximum possible fractional score achievable in the
 * remaining time.
 */
inline double calculate_optimistic_bound(uint64_t visited_mask,
                                         int current_time, int end_time_limit,
                                         const POI *pois,
                                         const int *sorted_pois_by_density,
                                         int min_transit_global, int n) {
    ...
    if (required_time <= remaining_time) {
      ...
      if (required_time > 0) {
        optimistic_future_score +=
            pois[i].score *
            (static_cast<double>(remaining_time) / required_time);
      }
      break;
    }
  }
 ...
 *
 * Explores the graph of POIs while strictly pruning branches that exceed
 * constraints (budget, deadlines, time windows) or that cannot possibly beat
 * the best known score.
 *
 * @param u The index of the current POI we are visiting.
 * @param state The current search state (mutated and reverted during
 * recursion).
 ...
 * @param sorted_pois_by_density Pre-computed POI indices sorted for the
 * knapsack heuristic.
 * @param best_result Reference to the global best result found so far.
 */
void dfs(int u, SearchState &state, const POI *pois,
         const TransitInfo *transit_times, const OptimizationConfig &config,
         const int *sorted_pois_by_density, const int *density_rank,
         int min_transit_global, int n, std::vector<TTBucket> &memo,
  ...
  if ((node_eval_count & 1023) == 0) {
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - start_time)
            .count() > config.timeout_ms) {
  ...
  if (!inserted) {
    if (bucket.count < TT_BUCKET_SIZE) {
      ...
      int evict_idx = 0;
      double min_score = bucket.entries[0].current_score;
      for (int i = 1; i < TT_BUCKET_SIZE; ++i) {
        if (bucket.entries[i].current_score < min_score) {
          min_score = bucket.entries[i].current_score;
          evict_idx = i;
        }
      }
  ...
  int effective_end_time = config.end_time_limit != -1
                               ? config.end_time_limit
                               : std::numeric_limits<int>::max();
  double max_possible_score =
      state.current_score +
      calculate_optimistic_bound(state.visited_mask, state.current_time,
                                 effective_end_time, pois,
  ...
  for (int i = 0; i < n; ++i) {
    if ((state.visited_mask & (1ULL << i)) == 0) {
      ...
      if (config.end_node_index.has_value() &&
          v == config.end_node_index.value()) {
        continue; // Do not visit the end node in the middle of the itinerary
      }

      // Calculate temporal feasibility
      int arrival_time_before_wait =
          state.current_time + transit_times[u * n + v].duration;
      ...
      if (config.end_node_index.has_value() &&
        ...
        if (time_after_visit + return_dur > pois[target_end].latest_time)
          continue;
        if (config.end_time_limit != -1 &&
            time_after_visit + return_dur > config.end_time_limit)
          continue;
      } else {
      ...
      bool is_rest_node = (pois[v].type == NodeType::BAR ||
                           pois[v].type == NodeType::HOTEL || is_strict_meal);
      if (is_rest_node) {
        next_continuous_active_time = 0;
      } else {
        if (state.continuous_active_time + transit_times[u * n + v].duration >
            config.max_active_time_before_fatigue) {
          node_score *= config.fatigue_penalty_multiplier;
        }
        next_continuous_active_time +=
            transit_times[u * n + v].duration + pois[v].duration;
      }

      uint8_t category_count =
          state.category_visits[static_cast<size_t>(pois[v].type)];
      if (category_count >= config.monotony_threshold) {
        node_score *= std::pow(config.monotony_multiplier,
                               category_count - config.monotony_threshold + 1);
      }
      ...
      bool acts_as_meal =
          is_strict_meal || is_breakfast || is_lunch || is_dinner;
      ...
      // Recurse deeper
      dfs(v, state, pois, transit_times, config, sorted_pois_by_density,
          density_rank, min_transit_global, n, memo, global_mandatory_mask,
          best_result, start_time, node_eval_count);
  ...
  } else if (config.end_time_limit != -1 &&
             final_time > config.end_time_limit) {
    valid_end_node = false;
  }

  // Verify final node matches required type if specified
  if (config.end_node_type.has_value() && state.current_path_size > 0) {
    int tail_node = (final_path_size > state.current_path_size)
  ...
  if (state.current_path_size > 0 && valid_end_node) {
    ...
    // Lexicographical optimization: Maximize Score -> Minimize Time -> Minimize
    // Cost
...
optimize_itinerary(const std::vector<POI> &pois,
  ...
  // 64-bit mask restricts problem size. Scaling beyond this requires a
  // vector<bool> or big integer mask.
  if (pois.size() > 64) {
  ...
  for (int start_node = 0; start_node < n; ++start_node) {
    ...
    if (pois[start_node].earliest_time + pois[start_node].duration >
    ...
    state.current_time =
        pois[start_node].earliest_time + pois[start_node].duration;
    ...
    bool is_strict_meal =
        (pois[start_node].type == NodeType::RESTAURANT_BREAKFAST ||
    ...
    } else if (pois[start_node].type == NodeType::BAR ||
               pois[start_node].type == NodeType::HOTEL) {
      state.continuous_active_time = 0;
    } else {
    ...
    if (config.breakfast_deadline != -1 &&
        state.current_time > config.breakfast_deadline && !state.had_breakfast)
      continue;
    if (config.lunch_deadline != -1 &&
        state.current_time > config.lunch_deadline && !state.had_lunch)
      continue;
    if (config.dinner_deadline != -1 &&
        state.current_time > config.dinner_deadline && !state.had_dinner)
      continue;

    try {
      // Begin recursive search from this starting node
      dfs(start_node, state, pois_ptr, transit_ptr, config, sorted_pois_ptr,
          density_rank_ptr, min_transit_global, n, memo, global_mandatory_mask,
          best_result, start_time, node_eval_count);
