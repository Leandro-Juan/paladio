...
struct POI {
    ...
    int earliest_time;      ///< Earliest allowable arrival time (in minutes from a baseline).
...
struct OptimizationConfig {
    ...
    std::optional<int> end_node_index;       ///< Optional required specific final node index (Round-trip).
    ...
    int max_idle_time = 45;                  ///< Maximum allowed waiting time before a POI opens.
 ...
 * @param transit_times Flattened 2D matrix (N x N) of transit durations and costs.
