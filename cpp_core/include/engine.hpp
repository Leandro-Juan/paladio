...
struct POI {
  ...
  POI(NodeType t, double c, double s, int e, int l, int d, bool m = false)
      : cost(c), score(s), earliest_time(e), latest_time(l), duration(d),
        type(t), is_mandatory(m) {
...
struct OptimizationConfig {
  ...
  std::optional<int>
      start_node_index; ///< Optional fixed starting node (by index).
  std::optional<NodeType>
      end_node_type; ///< Optional required type for the final node.
  ...
  int breakfast_deadline =
      -1; ///< Latest time by which a breakfast spot must be visited.
  int lunch_deadline =
  ...
  int max_active_time_before_fatigue =
      240; ///< Max active minutes before fatigue sets in.
  double fatigue_penalty_multiplier =
      0.6; ///< Score multiplier for POIs visited under fatigue.
  int min_meal_spacing = 180; ///< Minimum minutes required between meals.
  int monotony_threshold =
      2; ///< Threshold for diminishing returns per category.
...
struct OptimizationResult {
  std::vector<int>
      path; ///< Sequence of POI indices representing the optimal route.
