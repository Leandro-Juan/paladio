...
TEST_F(EngineStressTest, MaximumBitmaskIndex) {
    // We want to force the algorithm to visit node index 63 and verify it prunes/visits correctly
    const int N = 64;
    ...
    // Node 63 is the destination, but we give it a massive score so the algorithm WANTS to go there
