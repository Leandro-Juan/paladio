...
class AmadeusScraper:
    def __init__(self):
        try:
            ...
            logger.warning(f"Amadeus client init failed (missing AMADEUS_CLIENT_ID/SECRET?): {e}")
            self.amadeus = None
        ...
        try:
            ...
            if response.data:
                for hotel in response.data[:2]: # Get top 2
                    ...
                    hotels_data.append({
                        "name": name,
                        "financials": {"price_per_night": 100.0}, # Amadeus basic hotel search doesn't return price
