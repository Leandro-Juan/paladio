...
async def scrape_static(
    url: str, params: dict | None = None, custom_headers: dict | None = None
) -> dict:
    """
    ...
    async with httpx.AsyncClient(**client_kwargs) as client:
        try:
            ...
            for keyword in captcha_keywords:
                if keyword.lower() in text_content.lower():
                    raise BotDetectionError(
                        f"Bot detection triggered (found '{keyword}') for URL: {url}"
                    )
            ...
            logger.error(
                f"HTTP error {exc.response.status_code} while requesting {url}"
            )
            raise
