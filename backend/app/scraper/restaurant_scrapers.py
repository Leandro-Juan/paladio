...
class FoursquareScraper:
    ...
    async def scrape_restaurants(self, city: str, limit: int = 4) -> list[dict]:
        ...
        params = {"query": "restaurant", "near": city, "limit": limit}
        ...
        try:
            ...
            for place in data.get("results", []):
                ...
                restaurants.append(
                    {"name": name, "location": {"latitude": lat, "longitude": lon}}
                )
...
class OSMRestaurantScraper:
    async def scrape_restaurants(self, city: str, limit: int = 4) -> list[dict]:
        ...
        logger.info(
            f"Fetching restaurants via OpenStreetMap Nominatim for {city} with viewbox {viewbox}"
        )
