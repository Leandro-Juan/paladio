...
class BaseScraperStrategy:
    ...
    async def _check_bot_detection(self, page: Page, url: str):
        ...
        for _ in range(3):
            preview_text = await page.evaluate(
            ...
            for keyword in captcha_keywords:
                if (
                    ...
                    or keyword.lower() in raw_html_check.lower()
                ):
                    detected = True
                    break
            if detected:
                logger.info(
                    "Bot detection triggered. Waiting 5s for potential auto-solve..."
                )
        ...
        if detected:
            raise BotDetectionError(
                f"Bot detection triggered and not resolved for URL: {url}"
            )

        if len(raw_html_check) < 100 and "</body>" in raw_html_check:
            raise BotDetectionError(
                f"Bot detection triggered (Empty Body Drop) for URL: {url}"
            )
