...
from app.schemas.scraper import FoodAndDrink, Location, Schedule, MealSuitability, FoodFinancials, Scoring, Metadata
...
class RestaurantScraperStrategy(BaseScraperStrategy):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if "google.com/maps/search/" in url:
                    ...
                    raise DOMChangedError(f"Restaurant strategy for {url} not supported yet.")
    ...
    async def _scrape_tripadvisor(self, page, url):
        ...
        for idx, item in enumerate(page_data):
            rating_val = 0.0
            if item['rating'] != "0":
                try: rating_val = float(item['rating'].split()[0].replace(',', '.'))
                except: pass
