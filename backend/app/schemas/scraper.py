...
class Booking(BaseModel):
    provider_url: str = Field(description="URL to book the item, must include parameters for 2 adults")

class Metadata(BaseModel):
    scraped_at: datetime = Field(default_factory=datetime.utcnow, description="When this data was scraped")
    source: str = Field(description="Source of the data (e.g., 'Skyscanner', 'Booking.com')")
...
class Schedule(BaseModel):
    ...
    recommended_duration_minutes: int = Field(description="Recommended visit duration in minutes")
...
class FoodFinancials(BaseModel):
    price_tier: str = Field(description="Exact price tier string from source (e.g., '$', '$$', '$$$')")
    currency: str = Field(default="EUR", description="Currency (often inferred/default)")
...
class FoodAndDrink(BaseModel):
    ...
    category: str = Field(description="Main category (e.g. restaurant, cafe_bakery, bar)")
    ...
    scoring: Scoring
    cuisine: List[str] = Field(description="List of cuisines (e.g. ['Indian', 'Vegetarian'])")
    dietary_options: List[str] = Field(description="Dietary tags (e.g. 'Vegetarian friendly')")
    metadata: Metadata

class AttractionFinancials(BaseModel):
    is_free: bool = Field(description="Whether the attraction is completely free")
    estimated_cost: float | None = Field(description="Estimated cost of entry, if known")
    currency: str | None = Field(default="EUR", description="Currency of the estimated cost")

class AttractionSchedule(BaseModel):
    osm_opening_hours: str | None = Field(description="Raw OSM opening_hours string (e.g., 'Mo,We-Su 09:00-18:00')")
    recommended_duration_minutes: int = Field(description="Recommended visit duration in minutes")

class Attraction(BaseModel):
    id: str = Field(description="Unique identifier for the attraction (e.g. OSM node ID)")
    type: Literal["attraction"] = "attraction"
    category: Literal["museum", "monument", "historic_site", "landmark", "attraction"]
