...
class ScoringMLP(nn.Module):
    ...
    """
    @nn.compact
    def __call__(self, user_embedding: jnp.ndarray, poi_embedding: jnp.ndarray) -> jnp.ndarray:
...
@jax.jit
def update_user_embedding(params, user_embedding, poi_embedding, target_score, learning_rate=0.05):
    """
...
class JaxScoringModel(IScoringModel):
    ...
    def update_user(self, params: Any, user_embedding: Any, poi_embedding: Any, target_score: float, learning_rate: float = 0.05) -> Any:
        return update_user_embedding(params, user_embedding, poi_embedding, target_score, learning_rate)
