...
async def main():
    ...
    try:
        async with sse_client("https://mcp.price.win/mcp") as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                await session.initialize()
                print("Initialized. Searching for flights MAD to BCN on 2026-10-15...")
                
                # 1. Start the live search
                result = await session.call_tool("search_flights_live", arguments={
                    "origin": "MAD",
                    "destination": "BCN",
                ...
                poll_result = await session.call_tool("poll_flight_results", arguments={
                ...
                for content in poll_result.content:
                    if content.type == "text":
                        ...
                        if flights:
                            best = flights[0]
                            print(f"Cheapest flight: {best.get('airline')} - {best.get('price')} {best.get('currency')}")
                            print(f"Departure: {best.get('departure', {}).get('time')} -> Arrival: {best.get('arrival', {}).get('time')}")
