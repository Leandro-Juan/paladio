...
### `OptimizationConfig`
Defines the global user-provided constraints bounding the problem space. Passed down from the LangGraph Validator.
...
1. **Time Windows:** Arriving too late for a POI's `latest_time`.
...
- **Thread Safety:** The Global Interpreter Lock (GIL) is explicitly released via `py::gil_scoped_release release;` when calling `optimize_itinerary`. This enables Python to execute optimization requests on multiple threads concurrently, maximizing throughput in an async FastAPI environment.
- **Type Casting:** Python dictionaries and lists passed from `backend/app/engine/bridge.py` are automatically cast into C++ `std::vector` and `std::unordered_map` by pybind11. Ensure that the Python types perfectly match the C++ signatures to avoid segmentation faults.
